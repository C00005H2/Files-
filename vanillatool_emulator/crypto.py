"""Small, dependency-free AES-256-CBC codec used by the emulator.

VanillaTool uses the Windows CryptoAPI with an AES-256 plaintext key blob,
CBC mode, a fixed IV, and the CryptoAPI's final-block padding.  Keeping the
codec here avoids making the emulator depend on PyCryptodome or a system
OpenSSL command.
"""

from __future__ import annotations


# AES S-box and round constants.  These are the standard Rijndael tables.
_SBOX = (
    ,
)
_INV_SBOX = tuple(_SBOX.index(i) for i in range(256))
_RCON = (0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A)

BLOCK_SIZE = 16


def _xtime(value: int) -> int:
    value <<= 1
    if value & 0x100:
        value ^= 0x11B
    return value & 0xFF


def _multiply(left: int, right: int) -> int:
    result = 0
    while right:
        if right & 1:
            result ^= left
        left = _xtime(left)
        right >>= 1
    return result


def _expand_key(key: bytes) -> tuple[bytes, ...]:
    if len(key) != 32:
        raise ValueError("AES-256 requires a 32-byte key")

    # AES words are four bytes in column order.
    words = [list(key[offset : offset + 4]) for offset in range(0, len(key), 4)]
    for index in range(8, 60):
        word = words[index - 1][:]
        if index % 8 == 0:
            word = word[1:] + word[:1]
            word = [_SBOX[value] for value in word]
            word[0] ^= _RCON[index // 8]
        elif index % 8 == 4:
            word = [_SBOX[value] for value in word]
        words.append([left ^ right for left, right in zip(words[index - 8], word)])
    return tuple(bytes(words[offset] + words[offset + 1] + words[offset + 2] + words[offset + 3]) for offset in range(0, 60, 4))


def _add_round_key(state: list[list[int]], round_key: bytes) -> None:
    for column in range(4):
        for row in range(4):
            state[column][row] ^= round_key[column * 4 + row]


def _sub_bytes(state: list[list[int]], table: tuple[int, ...]) -> None:
    for column in range(4):
        for row in range(4):
            state[column][row] = table[state[column][row]]


def _shift_rows(state: list[list[int]], inverse: bool = False) -> None:
    original = [column[:] for column in state]
    for column in range(4):
        for row in range(4):
            offset = -row if inverse else row
            state[column][row] = original[(column + offset) % 4][row]


def _mix_columns(state: list[list[int]], inverse: bool = False) -> None:
    for column in range(4):
        a, b, c, d = state[column]
        if inverse:
            state[column] = [
                _multiply(a, 0x0E) ^ _multiply(b, 0x0B) ^ _multiply(c, 0x0D) ^ _multiply(d, 0x09),
                _multiply(a, 0x09) ^ _multiply(b, 0x0E) ^ _multiply(c, 0x0B) ^ _multiply(d, 0x0D),
                _multiply(a, 0x0D) ^ _multiply(b, 0x09) ^ _multiply(c, 0x0E) ^ _multiply(d, 0x0B),
                _multiply(a, 0x0B) ^ _multiply(b, 0x0D) ^ _multiply(c, 0x09) ^ _multiply(d, 0x0E),
            ]
        else:
            state[column] = [
                _xtime(a) ^ (_xtime(b) ^ b) ^ c ^ d,
                a ^ _xtime(b) ^ (_xtime(c) ^ c) ^ d,
                a ^ b ^ _xtime(c) ^ (_xtime(d) ^ d),
                (_xtime(a) ^ a) ^ b ^ c ^ _xtime(d),
            ]


def _encrypt_block(block: bytes, round_keys: tuple[bytes, ...]) -> bytes:
    state = [list(block[offset : offset + 4]) for offset in range(0, BLOCK_SIZE, 4)]
    _add_round_key(state, round_keys[0])
    for round_key in round_keys[1:-1]:
        _sub_bytes(state, _SBOX)
        _shift_rows(state)
        _mix_columns(state)
        _add_round_key(state, round_key)
    _sub_bytes(state, _SBOX)
    _shift_rows(state)
    _add_round_key(state, round_keys[-1])
    return bytes(value for column in state for value in column)


def _decrypt_block(block: bytes, round_keys: tuple[bytes, ...]) -> bytes:
    state = [list(block[offset : offset + 4]) for offset in range(0, BLOCK_SIZE, 4)]
    _add_round_key(state, round_keys[-1])
    for round_key in reversed(round_keys[1:-1]):
        _shift_rows(state, inverse=True)
        _sub_bytes(state, _INV_SBOX)
        _add_round_key(state, round_key)
        _mix_columns(state, inverse=True)
    _shift_rows(state, inverse=True)
    _sub_bytes(state, _INV_SBOX)
    _add_round_key(state, round_keys[0])
    return bytes(value for column in state for value in column)


def encrypt(plaintext: bytes, key: bytes, iv: bytes) -> bytes:
    """Encrypt bytes with AES-256-CBC and PKCS#7 padding."""
    if len(iv) != BLOCK_SIZE:
        raise ValueError("AES-CBC requires a 16-byte IV")
    padding = BLOCK_SIZE - (len(plaintext) % BLOCK_SIZE)
    padded = plaintext + bytes([padding]) * padding
    round_keys = _expand_key(key)
    previous = iv
    output = bytearray()
    for offset in range(0, len(padded), BLOCK_SIZE):
        block = bytes(left ^ right for left, right in zip(padded[offset : offset + BLOCK_SIZE], previous))
        previous = _encrypt_block(block, round_keys)
        output.extend(previous)
    return bytes(output)


def decrypt(ciphertext: bytes, key: bytes, iv: bytes) -> bytes:
    """Decrypt AES-256-CBC and validate/remove PKCS#7 padding."""
    if len(iv) != BLOCK_SIZE:
        raise ValueError("AES-CBC requires a 16-byte IV")
    if not ciphertext or len(ciphertext) % BLOCK_SIZE:
        raise ValueError("AES-CBC ciphertext must be a non-empty multiple of 16 bytes")
    round_keys = _expand_key(key)
    previous = iv
    output = bytearray()
    for offset in range(0, len(ciphertext), BLOCK_SIZE):
        block = ciphertext[offset : offset + BLOCK_SIZE]
        decoded = _decrypt_block(block, round_keys)
        output.extend(left ^ right for left, right in zip(decoded, previous))
        previous = block
    padding = output[-1]
    if padding < 1 or padding > BLOCK_SIZE or output[-padding:] != bytes([padding]) * padding:
        raise ValueError("invalid AES-CBC padding")
    return bytes(output[:-padding])


if __name__ == "__main__":  # pragma: no cover - handy for a quick manual check
    # AES-256-CBC NIST-style smoke test (the round trip is independent of the
    # emulator and catches accidental state-layout changes).
    _key = bytes(range(32))
    _iv = bytes(range(16))
    _message = b"VanillaTool emulator"
    assert decrypt(encrypt(_message, _key, _iv), _key, _iv) == _message
    print(encrypt(_message, _key, _iv).hex())
